...
func TradeOrderConsume(ctx context.Context, body string) error {
	...
	// 从数据库查询订单涉及的商品
	shopIdList := make([]int64, len(notice.Detail))
	orderCodeList := make([]string, len(notice.Detail))
	for i := 0; i < len(notice.Detail); i++ {
		detail := notice.Detail[i]
		shopIdList[i] = detail.ShopId
		orderCodeList[i] = detail.OrderCode
	}
